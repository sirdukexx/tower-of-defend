// ===================== Level 14 — Murkwood (2 lanes merging, dark swamp/forest) =====================
(function () {
  const W = 1280, H = 720;
  const S = 0.55;
  const LANE = 171 * S;
  ...
  function img(n) { const i = new Image(); i.src = 'assets/murkwood/' + n + '.png'; return i; }
  const ART = {
    bg: img('main_bg'), tree1: img('tree_1'), tree2: img('tree_2'),
    stone1: img('stone_1'), stone2: img('stone_2'), dot: img('dot'),
    hut: img('decor_1'), ribs: img('decor_2'), skull: img('decor_3'),
    grave1: img('decor_4'), grave2: img('decor_5'),
    BR: img('road_1'), BL: img('road_2'), TR: img('road_3'), TL: img('road_4'),
    V: img('road_5'), H: img('road_6'), merge: img('road_8'),
  };
  ...
  const A_TOP_Y = 95, A_TURN_X = 260;
  const B_TOP_Y = 65, B_TURN_X = 1080;
  const MERGE_ARMS_Y = 325;
  const MERGE_X = 660;
  ...
  const BUILD_SPOTS = [
    [460, 90], [900, 70], [80, 300], [1220, 260],
    [140, 460], [1160, 470], [420, 660], [1140, 660],
  ];
  ...
  function composeMap(m) {
    ...
    // AND every build pad (numeric check, not by eye) -- decor must never
    // sit on the road or block a tower spot
    const trees1 = [
      [40, 470], [1250, 460], [40, 660], [1250, 700], [909, 259],
    ];
    for (const [x, y] of trees1) sprite(m, ART.tree1, x, y, 0.55);
    const trees2 = [
      [568, 135], [710, 134], [300, 700], [812, 169],
    ];
    for (const [x, y] of trees2) sprite(m, ART.tree2, x, y, 0.5);
    const stones = [[610, 200], [743, 175], [168, 244], [1165, 350]];
    for (let i = 0; i < stones.length; i++) sprite(m, i % 2 ? ART.stone2 : ART.stone1, stones[i][0], stones[i][1], 0.85);
    sprite(m, ART.ribs, 812, 257, 0.55);
    sprite(m, ART.skull, 155, 561, 0.6);
    sprite(m, ART.grave1, 314, 432, 0.75);
    sprite(m, ART.grave2, 1181, 618, 0.7);
    sprite(m, ART.hut, 1035, 501, 0.5);
  }
  ...
  window.LEVELS.push({
    id: 'level14', name: 'Murkwood', desc: '2 ทาง · เดินผ่านหนองน้ำมืดแล้วมารวมกัน',
