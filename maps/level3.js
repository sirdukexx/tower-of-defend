// ===================== Level 3 — Triple Merge (3 lanes join, one exit) =====================
(function () {
  const W = 1280, H = 720;
  const S = 0.55;
  const LANE = 171 * S;
  ...
  const TOP_Y = 150, SPINE_X = 553, MID_Y = 406, TURN_X = 993;
  ...
  function composeMap(m) {
    ...
    const trees = [[140,320],[308,303],[414,335],[826,309],[700,85],[294,560],[420,640],[194,662],[1158,303],[1216,555]];
    for (const [x, y] of trees) sprite(m, ART.tree, x, y, 0.55);
    const stones = [[696,253],[979,251],[260,603],[700,560],[1136,543]];
    for (const [x, y] of stones) sprite(m, ART.stone, x, y, 0.7);
    sprite(m, ART.ruinR, 884, 85, 0.5);
    sprite(m, ART.ruinS, 103, 633, 0.5);
    sprite(m, ART.cabin, 860, 681, 0.62);
  }
  ...
  window.LEVELS.push({
    id: 'level3', name: 'Triple Merge', desc: '3 ทาง · รวมกันเป็น T แล้วออกทางเดียว',
